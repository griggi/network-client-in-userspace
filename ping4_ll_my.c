...
uint8_t *get_mac_from_string(char *);
...
main (int argc, char **argv)
{
  ...
  char *arg_mac_address = argv[1];

  src_mac = get_mac_from_string(arg_mac_address);
  ...
  //My router mac address b0:7f:b9:0d:ce:08
  //My router mac b0:7f:b9:0d:cd:88
  dst_mac[0] = 0xb0;
  dst_mac[1] = 0x7f;
  dst_mac[2] = 0xb9;
  dst_mac[3] = 0x0d;
...
// get mac address from string representation
uint8_t *
get_mac_from_string(char *mac_add_str){
  uint8_t *mac_add;
  mac_add = allocate_ustrmem(6);
  int curr_idx = 0;
  int last_valid_idx = -1;
  int i = 0;
  for (i = 0; i < strlen(mac_add_str); i++){
    if(mac_add_str[i] == ':'){
      int length = i - last_valid_idx - 1;

      char *byte_str;
      byte_str = (char *) malloc( ( length + 1) * sizeof(char));
      strncpy(byte_str,&mac_add_str[last_valid_idx+1],length);
      
      mac_add[curr_idx] = (uint8_t) strtol(byte_str, NULL, 16);
